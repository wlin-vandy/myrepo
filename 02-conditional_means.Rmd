...
pd<-pd%>%mutate(hs_dipl_level=ntile(hs_grad_pc,4))

pd<-pd%>%group_by(hs_dipl_level)%>%
mutate(pred_income_hs=mean(percapinc.2010))%>%
  ungroup

summary(pd$pred_income_hs)
```
...
rmse_cond_mean_hsgrads<-rmse(pd$percapinc.2010,pd$pred_income_hs)

rmse_cond_mean_hsgrads
