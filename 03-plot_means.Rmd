...
```{r}
gg<-ggplot(at,aes(x=Department,fill=Department)) #fill is color
gg<-gg+geom_bar()
gg
``` 
...
gg<-gg+geom_histogram(binwidth = 1) # add to fill="lightblue" to change color
gg
``` 
...
## Changing bandwidth-- not recommended, just showing you how. Smoothes out the
...
#We can ask R to tell us what the variale is, but since R thinks it's just a character, we can recode, table it, give levels of it
...
at_department<-at%>%
  group_by(Department)%>%
  summarise(attrit_avg=mean(attrit))

at_department

at%>% count(Department,attrit) %>%
  group_by(Department) %>%
  mutate(prop=prop.table(n)) %>%
  select(-n) %>%
  spread(attrit, prop)%>%kable()

gg_deparmtent <- ggplot(at_department,aes(x=Department,y=attrit_avg))
gg_deparmtent <- gg_deparmtent+geom_bar(stat="identity", fill="blue")
gg_deparmtent <- gg_deparmtent+xlab("Department")+ylab("Yearly Attrition")
gg_deparmtent
...
at_sum<-at%>%
  group_by(BusinessTravel,Gender)%>% #add another variable - Gender
  summarize(attr_avg=mean(attrit))
...
gg<-gg+geom_bar(stat="identity",aes(fill=Gender),position="dodge") #without position="dodge" it becomes a stacked bar chart
...
gg_deptedu <- ggplot(at_deptedu,aes(x=fct_reorder(Department,attrit_avg),y=attrit_avg,color=EducationField))
gg_deptedu <- gg_deptedu + geom_bar(stat="identity",aes(fill=EducationField), position = "dodge")
gg_deptedu <- gg_deptedu +xlab("Department") + ylab("pr(Attrition)")
gg_deptedu
...
gg_multivar<-ggplot(at_multivar,aes(x=fct_reorder(EducationField,attr_avg),y=attr_avg))
gg_multivar<-gg_multivar+geom_bar(stat = "identity", aes(fill=Gender),position="dodge")
gg_multivar<-gg_multivar+facet_wrap(~Department, ncol = 4)

gg_multivar<-gg_multivar+xlab("Education Field")+ylab("pr(Attrition)")
gg_multivar<-gg_multivar+theme(axis.text.x = element_text(angle = 60, hjust = 1))
gg_multivar<-gg_multivar+ggtitle("Departure by Department and Education Field")
gg_multivar
